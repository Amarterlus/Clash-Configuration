...
// @compatible  Firefox
// @license     MIT
// @downloadURL https://update.greasyfork.org/scripts/495368/TwitterX%20Media%20Downloader.user.js
// @updateURL https://update.greasyfork.org/scripts/495368/TwitterX%20Media%20Downloader.meta.js
// ==/UserScript==
/* jshint esversion: 8 */
...
const TMD = (function () {
    ...
    return {
        ...
        click: async function (btn, status_id, is_exist, index) {
            ...
            if (!medias) {
                medias = [];
                let entities = JSON.parse(json.card.legacy.binding_values[0].value.string_value).media_entities;
                console.log('========', entities)
                for (let key in entities) {
                    medias.push(entities[key]);
                }
            }
...
`);
        ...
        fetchJson: async function (status_id) {
            ...
            let tweet_entrie = tweet_detail.data.threaded_conversation_with_injections_v2.instructions.find(n => n.type == 'TimelineAddEntries').entries.find(n => n.entryId == `tweet-${status_id}`);
